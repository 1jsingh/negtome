...
class NegtomeArgs:
    ...
    merging_threshold: float = 0.7
    merging_dropout: float = 0.0
    merging_t_start: int = 1000
    merging_t_end: int = 800
    blocks: Optional[List[str]] = field(default_factory=lambda: ['up_blocks', 'mid_block', 'down_blocks'][:1]) # blocks to apply negtome
    min_attn_dim: int = 32 # min dimension of the layers for applying tokern merging
...
##########################################################################
def check_block_condition(name, negtome_args):
    for block_name in negtome_args['blocks']:
        if name.startswith(block_name): return True
    return False

def get_initial_crossframe_attn_mask(num_frames, sequence_length, rag_mask, negtome_args, device):
    ...
    dropout_mask = torch.rand(sequence_length, dtype=torch.float32, device=device).unsqueeze(1) >= negtome_args['neg_selfattn_dropout']
...
##########################################################################
# NegToMeAttnProcessor
##########################################################################
class NegToMeAttnProcessor2_0:
    r"""
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # negative token merging (negtome is added as a simple function after attention computation)
        if negtome_args is not None and H >= negtome_args['min_attn_dim'] and W >= negtome_args['min_attn_dim'] and check_block_condition(self.name, negtome_args):
            if negtome_args['use_negtome']:
                if negtome_args['timestep']<=negtome_args['merging_t_start'] and negtome_args['timestep']>=negtome_args['merging_t_end']:
                    if negtome_args['use_rag_assets']:
                        ...
                        mask = None
                    alpha = negtome_args['merging_alpha']
                    hidden_states_pos, hidden_states_neg = (hidden_states[:B//2], hidden_states[B//2:])
                    hidden_states_pos = negtome(hidden_states_pos, alpha=alpha, threshold=negtome_args['merging_threshold'], dropout=negtome_args['merging_dropout'], mask=mask)
                    hidden_states_neg= negtome(hidden_states_neg, alpha=alpha, threshold=negtome_args['merging_threshold'], dropout=negtome_args['merging_dropout'], mask=mask)
...
##########################################################################
# SDXL negtome Pipeline
##########################################################################
class StableDiffusionXLNegToMePipeline(
    DiffusionPipeline,
    ...
    def __init__(
        ...
        # set attn processor to use NegToMeAttnProcessor2_0 instead of AttnProcessor2_0()
    ...
    def set_attn_processor(self):
        ...
        for name in self.unet.attn_processors.keys():
            processor = NegToMeAttnProcessor2_0(name) # AttnProcessor2_0()
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if cross_attention_kwargs['negtome_args'] is not None and cross_attention_kwargs['negtome_args']['use_rag_assets']:
            ...
            rag_img, rag_mask = cross_attention_kwargs['negtome_args']['rag_assets'][0]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # cross_attention_kwargs['negtome_args'] = cross_attention_kwargs['negtome_args'] or {}
                if cross_attention_kwargs['negtome_args'] is not None: 
                    cross_attention_kwargs['negtome_args']['timestep'] = t.item()
